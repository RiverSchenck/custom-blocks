...
export const AnExampleBlock: FC<BlockProps> = ({ appBridge }) => {
    ...
    const rps = (speed?: string, reverse?: boolean, fallback = 100) => {
        ...
        const magnitude = Number.isFinite(n) ? n : fallback;
    ...
    const bg = blockSettings?.colorInput ? rgbObjectToRgbString(blockSettings.colorInput) : '#ffffff';
    ...
    const mvProps: JSX.IntrinsicElements['model-viewer'] = {
    ...
    return (
        <div style={{ width: '100%', height: 500 }}>
            {fileUrl ? <model-viewer ref={mvRef} {...mvProps} /> : <p>No supported 3D file selected (.glb required)</p>}
        </div>
    );
};
