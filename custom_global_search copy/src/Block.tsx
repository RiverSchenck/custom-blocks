...
export const CustomSearch: FC<BlockProps> = ({ appBridge }) => {
    ...
    const searchGlobal = async () => {
        ...
        const tempBrands = [
            { id: 'eyJpZGVudGlmaWVyIjoyMjE4LCJ0eXBlIjoiYnJhbmQifQ==', name: 'River Sandbox', isChecked: true },
        ];
        ...
        for (const brand of tempBrands) {
            ...
            if (brand.isChecked) {
                try {
                    ...
                    const items = data?.data?.brand?.search?.items;

                    if (items) {
                        const assetsToPush = items.slice(0, remaining); // Only take the number of items needed to reach maxResults
                        console.log(assetsToPush);
    ...
    return (
        <>
            ...
            {fetchedAssets.map((brandAsset, index) => (
                <BrandAssets key={index} brandName={brandAsset.brandName} assets={brandAsset.assets} />
            ))}
        </>
    );
};
