...
import { IconCheckMark, IconGridRegular, IconMagnifier, IconStackVertical } from '@frontify/fondue';
import { IconSize } from '@frontify/fondue';
...
export const BrandColorPicker = ({ palettes: defaultPalettes = [], currentColors, onSelect }: Props): ReactElement => {
    const views = [
            ...
            <ul className="tw-flex tw-flex-col tw-gap-4">
                {palettesWithColors.length > 0
                    ? palettesWithColors.map(({ id, title, colors }) => (
                          <li key={`${id}-${generateRandomId()}`} className="tw-flex tw-flex-col tw-gap-2">
                              ...
                              >
                                  {colors.map((color) => (
                                      <li key={`${color.name}-${generateRandomId()}`} data-test-id="brand-color">
                                          <button
                                          ...
                                          >
                                              ...
                                              >
                                                  {isColorSelected(color) && <IconCheckMark size={IconSize.Size20} />}
                                              </span>
